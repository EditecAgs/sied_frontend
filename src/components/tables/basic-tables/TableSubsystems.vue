	...
	<div class="bg-white rounded-xl shadow-lg overflow-hidden">
		<div class="px-6 py-4 bg-gradient-to-r from-brand-800 to-brand-900">
			<div class="flex justify-between items-center">
				<h2 class="text-xl font-bold text-white">Gestión de Subsistemas</h2>
				<button
			...
			<table class="min-w-full">
				<thead>
					<tr class="bg-brand-800/80 text-white">
						<th class="px-5 py-3 text-left text-sm font-semibold border-b border-brand-700/50 border-r border-brand-700/30">
							Nombre del Subsistema
						</th>
						<th class="px-5 py-3 text-left text-sm font-semibold border-b border-brand-700/50">
							Opciones
						</th>
					</tr>
				...
				<tbody>
					...
					<tr
						v-for="(subsystem, index) in paginatedSubsystems"
						...
						<td class="px-5 py-3 text-sm border-r border-gray-100">
							<span class="block font-medium text-gray-800">{{ subsystem.name }}</span>
						</td>
					...
					<tr v-if="!isLoading && filteredSubsystems.length === 0">
						<td colspan="2" class="px-5 py-8 text-center">
							<p class="text-gray-500">No se encontraron subsistemas</p>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="px-6 py-3 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
			<span class="text-xs text-gray-600">
				Mostrando {{ paginatedSubsystems.length }} de {{ filteredSubsystems.length }} registros
			</span>
...
const totalPages = computed(() => Math.ceil(filteredSubsystems.value.length / rowsPerPage.value));

const paginatedSubsystems = computed(() => {
	const start = (currentPage.value - 1) * rowsPerPage.value;
	return filteredSubsystems.value.slice(start, start + rowsPerPage.value);
});
