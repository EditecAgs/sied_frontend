	...
	<div class="bg-white rounded-xl shadow-lg overflow-hidden">
		<div class="px-6 py-4 bg-gradient-to-r from-brand-800 to-brand-900">
			<div class="flex justify-between items-center">
				<h2 class="text-xl font-bold text-white">Gestión de Sectores</h2>
				<button
			...
			<table class="min-w-full">
				...
				<tr class="bg-brand-800/60 text-white">
					<th class="px-5 py-2 border-b border-brand-700/50 border-r border-brand-700/30">
						<input v-model="filters.name" class="w-full bg-white/10 border-none text-white placeholder-white/60 rounded px-3 py-1 text-xs focus:outline-none" placeholder="Buscar sector..." />
					</th>
				...
				<tr v-if="isLoading">
					<td colspan="2" class="py-12 text-center">
						...
						<p class="text-gray-500">Cargando Sectores...</p>
					</td>
				</tr>
				...
				<tr
					...
					<td class="px-5 py-3 text-sm">
						<div class="flex space-x-2">
							<btnEdit :table="'sectors'" :pk="sector.id ?? index" @open="(data) => $emit('open', data)" />
							<btnDelete :table="'sectors'" :pk="sector.id ?? index" @open-confirm="(payload) => $emit('open-confirm', payload)" />
						</div>
					</td>
				</tr>

				<tr v-if="!isLoading && filteredSectors.length === 0">
					<td colspan="2" class="px-5 py-8 text-center">
						<p class="text-gray-500">No se encontraron sectores registrados</p>
					</td>
				</tr>
				</tbody>
			</table>
      ...
      <span class="text-xs text-gray-600">
        Mostrando {{ paginatedSectors.length }} de {{ filteredSectors.length }} registros
      </span>
...
const filteredSectors = computed(() => {
	return sectors.value.filter(sector => {
		return sector.name.toLowerCase().includes(filters.value.name.toLowerCase());
	});
});

const totalPages = computed(() => Math.ceil(filteredSectors.value.length / rowsPerPage.value));
const paginatedSectors = computed(() => {
	const start = (currentPage.value - 1) * rowsPerPage.value;
	return filteredSectors.value.slice(start, start + rowsPerPage.value);
});

watch([filteredSectors, rowsPerPage], () => {
