	...
	<div class="bg-white rounded-xl shadow-lg overflow-hidden">
		<div class="px-6 py-4 bg-gradient-to-r from-brand-800 to-brand-900">
			<div class="flex justify-between items-center">
				<h2 class="text-xl font-bold text-white">Gestión de Estados de Documentos</h2>
				<button
				...
				<thead>
					...
					<tr class="bg-brand-800/60 text-white">
						<th class="px-5 py-2 border-b border-brand-700/50 border-r border-brand-700/30">
							<input v-model="filters.name" class="w-full bg-white/10 border-none text-white placeholder-white/60 rounded px-3 py-1 text-xs focus:outline-none" placeholder="Buscar estado..." />
						</th>
				...
				<tbody>
					<tr v-if="isLoading">
						<td colspan="2" class="py-12 text-center">
							...
							<p class="text-gray-500">Cargando Estados de Documentos...</p>
						</td>
					</tr>

					<tr
						v-for="(status, index) in paginatedStatuses"
						:key="status.id ?? index"
						...
						<td class="px-5 py-3 text-sm">
							<div class="flex space-x-2">
								<btnEdit :table="'Estatus de Documento'" :pk="status.id ?? index" @open="(data) => $emit('open', data)" />
								<btnDelete :table="'document_statuses'" :pk="status.id ?? index" @open-confirm="(payload) => $emit('open-confirm', payload)" />
							</div>
						</td>
					</tr>

					<tr v-if="!isLoading && filteredStatuses.length === 0">
						<td colspan="2" class="px-5 py-8 text-center">
							<p class="text-gray-500">No se encontraron estados de documentos registrados</p>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div v-if="!isLoading" class="px-6 py-3 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
			<span class="text-xs text-gray-600">
				Mostrando {{ paginatedStatuses.length }} de {{ filteredStatuses.length }} registros
			</span>
...
const filteredStatuses = computed(() => {
	return statuses.value.filter(status => {
		return status.name.toLowerCase().includes(filters.value.name.toLowerCase());
	});
});

const totalPages = computed(() => Math.ceil(filteredStatuses.value.length / rowsPerPage.value));
const paginatedStatuses = computed(() => {
	const start = (currentPage.value - 1) * rowsPerPage.value;
	return filteredStatuses.value.slice(start, start + rowsPerPage.value);
});

watch([filteredStatuses, rowsPerPage], () => {
