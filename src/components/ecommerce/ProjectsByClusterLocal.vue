	...
	<div
		...
		<div class="flex justify-between items-center mb-4 sm:mb-6">
			...
			<div v-else class="text-xs text-stone-50 dark:text-gray-300 sm:text-sm">
				{{ filteredLocalClusters.length }} cámaras
			</div>
		</div>
		...
		<div class="relative min-h-[300px] sm:min-h-[350px]">
			...
			<div v-else-if="!hasData" class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-gray-900/80 z-10 rounded-lg">
				<div class="text-center">
					...
					<p class="text-xs text-stone-50/80 dark:text-gray-400 mt-1">No se encontraron proyectos en cámaras locales</p>
				</div>
			</div>
			...
			<div v-else class="space-y-4 sm:space-y-6">
				...
				<div class="mt-3 text-center text-stone-600 dark:text-gray-300 text-xs sm:text-sm">
					Total: {{ totalProjectsLocal }} proyectos •
					{{ filteredLocalClusters.length }} cámaras locales con proyectos
					<span v-if="localClustersWithoutProjects > 0" class="block text-xs text-stone-600/80 dark:text-gray-400 mt-1">
						{{ localClustersWithoutProjects }} cámaras sin proyectos
					</span>
				</div>
			</div>
		</div>
...
const localClustersWithoutProjects = computed(() => {
	return localClusters.value.filter(item => item.project_count === 0).length
})

const totalProjectsLocal = computed(() => {
	return filteredLocalClusters.value.reduce((sum, item) => sum + item.project_count, 0)
})

const hasData = computed(() => {
	return filteredLocalClusters.value.length > 0
})

const chartData = computed(() => {
	const displayData = filteredLocalClusters.value
	...
	return {
		...
		datasets: [
			{
				label: 'Proyectos por Cámara Local',
		...
		tooltip: {
			callbacks: {
				title: function(context) {
					...
					const fullName = filteredLocalClusters.value[index]?.cluster_name || ''
				...
				label: function(context) {
					...
					const total = totalProjectsLocal.value
...
const fetchData = async () => {
	try {
		...
		if (response && response.locales !== undefined) {
			clustersData = response
		}
		else if (response && response.data && response.data.locales !== undefined) {
			clustersData = response.data
		}
		else if (response && response.data && response.data.data && response.data.data.locales !== undefined) {
