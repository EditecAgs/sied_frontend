...
import { showDocumentStatus } from "../../../services/dual_projects/documents-statuses"

const emit = defineEmits(['close', 'saved'])
const isLoading = ref(false)
const alvRoute = ref()
alvRoute.value = axios.defaults.baseURL + 'documents-statuses'
...
watchEffect(() => {
	if (props.data.mode === 'edit' && props.data.pk !== null) {
		alvRoute.value = `${axios.defaults.baseURL}documents-statuses/${props.data.pk}`
		alvMethod.value = 'PUT'
		isLoading.value = true

		showDocumentStatus(props.data.pk).then(res => {
