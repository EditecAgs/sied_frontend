		...
		<div
			...
			<div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 relative max-h-[85vh] flex flex-col overflow-hidden">
				...
				<h4 v-if="!success && !error" class="text-3xl font-extrabold text-brand-900 mb-4 text-center">
					¿Estás segura/o de eliminar este diplomado?
				</h4>
...
import { removeDiplomas } from "../../../services/dual_projects/diplomas";   
...
function deleteItem() {
	...
	removeDiplomas(props.pk)       
		.then(() => {
			success.value = 'Diplomado eliminado con éxito.'
			loading.value = false
			setTimeout(() => {
		...
		.catch(() => {
			error.value = 'Hubo un error al eliminar el diplomado. Intenta de nuevo.'
			loading.value = false
		})
}
