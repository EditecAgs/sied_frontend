		...
		<div
			...
			<div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 relative max-h-[85vh] flex flex-col overflow-hidden">
				...
				<h4 v-if="!success && !error" class="text-3xl font-extrabold text-brand-900 mb-4 text-center">
					¿Estás segura/o de eliminar esta certificación?
				</h4>
...
function deleteItem() {
	...
	removeCertification(props.pk)
		.then(() => {
			success.value = 'Certificación eliminada con éxito.'
			loading.value = false
			setTimeout(() => {
		...
		.catch(() => {
			error.value = 'Hubo un error al eliminar la certificación. Intenta de nuevo.'
			loading.value = false
		})
}
